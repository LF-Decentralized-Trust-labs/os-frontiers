# ORF Instrument Catalog

> **The Complete Portfolio of Open Replenishment Framework Collection Instruments**  
> *Excerpted from the Open Replenishment Framework (ORF) Research Candidate v0.8.0-rc.1*

---

## Maturity Legend
- 🟢 **Live Precedent**: Materially similar mechanism is operating in production today.
- 🟡 **Partial Analog**: One or more components proven, but the complete ORF integration requires piloting.
- 🔵 **Research-Stage**: Needs technical, legal, economic, or governance validation before deployment.

---

## Master Catalog Matrix

| Instrument Name | Layer | Fork-Resistance Anchor | Precedent & Evidence Status | Implementation Tier |
|---|---|---|---|---|
| **Protocol Fee Routing** | Protocol | Canonical ledger rules | 🟢 Cardano L1 transaction fee split (`tau`) | Tier 1 (0-18m) |
| **Monetary Expansion Allocation** | Protocol | Canonical ledger rules | 🟡 Transitional protocol issuance (Does not count toward Net Replenishment Ratio) | Tier 1 (Transitional) |
| **Surplus Sequencer / Priority Tithe** | Protocol | Sequencing and ledger activity | 🟢 Optimism sequencer allocation / RetroPGF | Tier 3 (36m+) |
| **Slashing & Penalty Routing** | Protocol | Canonical ledger rules | 🟢 Polkadot directs portion of slashes to Treasury | Tier 3 (36m+) |
| **Certification-Gated Contribution** | Application | Brand, registry, support bundle | 🟡 Protocol Guild + Tidelift partial analog | Tier 1 (0-18m) |
| **Dependency-Directed Revenue Share** | Application | Verified dependency graph & state | 🟡 Drips live graph; tea proof of contribution | Tier 2 (18-36m) |
| **Verifiable Attestation Pools** | Application | Attested graph & reproducible builds | 🔵 Drips / ZK attestation research (Downgraded to Research-Stage) | Tier 2 (18-36m) |
| **AI-Assisted Dependency Allocation** | Application | Verified graph, model competition | 🟡 Deep Funding pilot model | Tier 2 (18-36m) |
| **Streaming Revenue Distribution** | Application | On-chain stream contracts & consent | 🟢 Superfluid infrastructure live | Tier 2 (18-36m) |
| **Ecosystem-Owned Services** | Application | Canonical service & network effects | 🟢 ENS registration revenue to DAO treasury | Tier 2 (18-36m) |
| **Maintenance SLAs & LTS Services** | Enterprise | Relationships, assurance, release commitments | 🟢 Red Hat Extended Lifecycle (ELC) & Tidelift | Tier 1 (0-18m) |
| **Supply-Chain SLA + Reciprocal Clause** | Enterprise | Assurance relationship & contract right | 🟡 Tidelift / Red Hat + recoverable funding | Tier 1 (0-18m) |
| **Training & Certification** | Enterprise | Brand & credential verification value | 🟢 Linux Foundation Education / Training & Certification | Tier 1 (0-18m) |
| **Foundation-Hosted Consortium** | Enterprise | Neutral brand, membership, hosting | 🟢 Linux Foundation neutral hosting (Apache explicitly non-SLA) | Tier 1 (0-18m) |
| **Recoverable / Reciprocal Funding** | Enterprise | Contract & treasury repayment rights | 🟡 Program-Related Investments (U.S. private foundation structure) | Tier 1 (0-18m) |
| **Governed Endowment Policy** | Capital | Treasury & IPS authority | 🟢 ENS Investment Policy Statement (EP 6.46); Octant yield routing | Tier 1 (IPS) / Tier 3 |
| **Perpetual Auction Treasury Funding** | Capital | Auction contract, brand, collector demand | 🟢 Nouns daily 24h auction | Tier 2 (18-36m) |
| **Autonomous Revenue Networks** | Capital | Immutable treasury & redemption rules | 🔵 Revnet emerging architecture | Tier 3 (36m+) |
| **Yield-Donating Settlement Assets** | Capital | Reserve yield & payment asset adoption | 🟢 Glo Dollar stablecoin model | Tier 2 (18-36m) |
| **Strategic Venture Positions** | Capital | Treasury equity or warrants | 🔵 Mission investment analog (High Risk) | Tier 3 (36m+) |
| **Infrastructure Bonds** | Capital | Future governed fee revenue | 🔵 Revenue-bond analog (Research Stage) | Tier 3 (36m+) |
| **Security Mutual Premiums** | Capital | Risk pool & underwriting capacity | 🟢 Nexus Mutual capital pool | Tier 2 (18-36m) |
| **Hybrid Yield + Risk Pool** | Capital | Governed IPS, reserves, underwriting | 🟡 ENS Endowment + Nexus Mutual hybrid | Tier 2 (18-36m) |
| **Public-Goods Stake Pools** | Delegation | Stake, certification, wallet discovery | 🟡 Cardano mission-driven pools (Voluntary operator reward donations) | Tier 1 (0-18m) |
| **Cross-Chain Delegation Registry** | Delegation | Verifiable commitments, discovery | 🔵 Cross-chain stake discovery proposal (Research Stage) | Tier 2 (18-36m) |

---

## Layer-by-Layer Instrument Breakdown

### 1. Protocol Layer (Automatic Capture)
Instruments encoded directly into core consensus, block building, or sequencing rules. They capture value automatically from ledger activity without requiring per-transaction user consent.
* **Key Precedents**: Cardano Monetary Policy, Optimism Sequencer Tithe, Polkadot Slashing.
* **Governance Guardrail**: Must carry constitutional community mandates due to their proximity to taxation.

### 2. Application Layer (Incentivized Opt-In)
Instruments that capture value from applications operating on top of shared infrastructure.
* **Flagship Proposal**: "Sustains the Commons" Certification. Apps that route a disclosed fee share to the treasury receive official badge placement, priority support, and registry verification.
* **Dependency Metering**: Drips graph splitting and tea attestation pools.

### 3. Enterprise & Services Layer (Earned Revenue)
Commercial products sold directly to enterprises, denominated in fiat or stablecoins.
* **Core Products**: Maintenance SLAs, Extended Lifecycle Support (LTS) patch windows, dependency security intelligence, corporate training.
* **Reciprocal Clauses**: Commercial grants that convert into capped revenue shares or repayable advances upon commercial success.

### 4. Capital Layer (Productive Treasury)
Transforms static treasury reserves into active, yield-generating income assets governed by an **Investment Policy Statement (IPS)**.
* **Core Products**: Governed Endowments (ENS IPS), Yield-donating stablecoins (Glo Dollar), Security Mutuals (Nexus Mutual).

### 5. Delegation Layer (Ecosystem Extension)
Behavioral layer for Proof-of-Stake networks where pool operators pledge a portion of margin rewards to public-goods maintenance.
